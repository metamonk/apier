...
This project uses a branch-based deployment strategy with three environments:
...
- **Production** (`main` branch)
  ...
  - Purpose: Customer-facing API with real data
...
- **Sandbox** (`sandbox` branch)
  ...
  - Deploy: Push to `sandbox` branch
  - Stack name: Auto-generated by Amplify
  - Purpose: Demos, experiments, third-party integrations
  - Documentation: [docs/SANDBOX.md](./docs/SANDBOX.md)
...
2. **Sandbox Testing** (optional for isolated testing):
   ```bash
   git checkout sandbox
   # Make changes or merge from dev
   git add .
   git commit -m "test: experiment with new feature"
   git push origin sandbox
   ```
   → Triggers automatic deployment to sandbox environment
...
2. **Configure Branches**:
   ...
   - Add `sandbox` branch → Sandbox environment (for testing)

3. **Build Settings**:
   - Amplify will auto-detect the `amplify.yml` configuration
   - All branches use the same build configuration
